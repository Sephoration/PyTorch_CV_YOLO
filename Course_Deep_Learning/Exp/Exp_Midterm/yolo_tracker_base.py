...
class YOLOTracker:
    ...
    def __init__(self, model_path=None, device=None):
        ...
        # ========== Part B: 监控区间参数 ==========
        self.valid_y_min = 0        # 有效Y轴范围最小值（上边界）- 默认全画面
        self.valid_y_max = 9999     # 有效Y轴范围最大值（下边界）- 默认全画面
        ...
        self.short_acc_threshold = 20      # 累积角度阈值（降低以适应远离镜头的变道）
        self.short_consistent_ratio = 0.50  # 方向一致性比例（降低）
        ...
        # ========== Part B: 横向位移确认参数 ==========
        self.trajectory_window_size = 25   # 最近25个轨迹点
        self.min_lateral_shift = 15        # 横向总位移至少15像素（降低以适应远处车辆）
        self.min_lateral_ratio = 0.15      # 横向/纵向比例至少0.15（降低）
        self.min_x_consistent_ratio = 0.55  # 横向方向一致性至少55%（降低）
        
        # ========== Part C: 净角度变化检查参数 ==========
        self.short_net_threshold = 12      # 短窗口净角度门槛（降低）
        self.long_net_threshold = 18       # 长窗口净角度门槛（降低）
        ...
        # ========== Part C: 异常角度过滤参数 ==========
        self.abnormal_angle_diff_threshold = 8.0  # 单步角度差超过8度视为异常（放宽）
        
        # ========== 新增：速度过滤 ==========
        self.min_speed = 2.0              # 最小速度（像素/帧），低于此值跳过变道分析
        
        # ========== 新增：横向位移轴方向 ==========
        self.lateral_axis = 'x'           # 横向位移轴：'x'=垂直道路(上下行)，'y'=水平道路(左右行)
    ...
    def draw_valid_y_zone(self, frame, color=(0, 255, 255), thickness=2):
        ...
        h, w = frame.shape[:2]
        if self.valid_y_min > 0:
            cv2.line(frame, (0, int(self.valid_y_min)), (w, int(self.valid_y_min)), color, thickness)
            cv2.putText(frame, f"Y_MIN={self.valid_y_min}", (20, int(self.valid_y_min) - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        if self.valid_y_max < h:
            cv2.line(frame, (0, int(self.valid_y_max)), (w, int(self.valid_y_max)), color, thickness)
            cv2.putText(frame, f"Y_MAX={self.valid_y_max}", (20, int(self.valid_y_max) - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
        return frame
    ...
    def judge_lateral_shift_by_trail(self, trail):
        ...
        if self.lateral_axis == 'y':
            lateral_disp = dy
            forward_disp = dx
            axis_diffs = [recent_trail[i][1] - recent_trail[i-1][1] for i in range(1, len(recent_trail))]
        else:
            lateral_disp = dx
            forward_disp = dy
            axis_diffs = [recent_trail[i][0] - recent_trail[i-1][0] for i in range(1, len(recent_trail))]
        
        if abs(lateral_disp) < self.min_lateral_shift:
            return False
        
        lateral_ratio = abs(lateral_disp) / (abs(forward_disp) + 1e-6)
        ...
        positive_count = sum(1 for d in axis_diffs if d > 0)
        negative_count = sum(1 for d in axis_diffs if d < 0)
        
        if lateral_disp > 0:
            consistent_ratio = positive_count / len(axis_diffs)
        elif lateral_disp < 0:
            consistent_ratio = negative_count / len(axis_diffs)
        else:
            consistent_ratio = 0
        
        return consistent_ratio >= self.min_x_consistent_ratio
            ...
            for box in results[0].boxes:
                ...
                if analyze_lane_change:
                    ...
                    if len(trail_to_use) >= self.angle_step + 1:
                        ...
                        too_slow = motion_mag < self.min_speed
