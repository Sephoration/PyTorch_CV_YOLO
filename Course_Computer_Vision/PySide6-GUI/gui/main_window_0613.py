...
class VisionWorkstationGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.init_yoga_tab_ui()  # 把 UI 逻辑独立成一个函数，保持代码整洁
        self.param_tabs.addTab(self.yoga_tab, "瑜珈姿势检测")

        # ==================== Tab 5: YOLO 参数配置 ====================
        self.yolo_tab = QWidget()
        self.init_yolo_tab_ui()   # 🎯 独立函数呼叫：UI 解耦最佳实践
        ...
        # 🎯 物理对称改造：植入真实的隐藏按钮，强制左右高度 100% 一致
        ...
        self.view_raw.setMinimumSize(320, 240)  # 🎯 新增：赋予它能够缩小的能力
        # self.view_raw.setScaledContents(True) # ❌ 删掉或注释掉这行
        ...
        self.right_video_box.setContentsMargins(0, 0, 0, 0)  # <--- 新增这行，消除右侧默认的外边距
        ...
        self.view_processed.setMinimumSize(320, 240)  # 🎯 新增：赋予它能够缩小的能力
        # self.view_processed.setScaledContents(True) # ❌ 删掉或注释掉这行
        ...
        # # ----------------------------------------------------
        # # 🎯 UX 优化：通过 QTabBar 设定已完成与未完成的页签颜色
        # # ----------------------------------------------------
        # tab_bar = self.param_tabs.tabBar()
        #
        # # 设定颜色规范 (可以使用 16 进制色码)
        # color_active = QColor("#005577")  # 深蓝色：代表已实作完成并可用
        # color_todo = QColor("#999999")  # 浅灰色：代表尚在开发中的占位符

        # ----------------------------------------------------
        # 🎯 UX 优化：利用 Emoji 直觉区分开发进度，完全无视系统主题差异！
        # ----------------------------------------------------
        # 直接修改 Tab 的标题文字，加入视觉符号
        ...
        self.param_tabs.setTabText(5, "✅ YOLO 参数配置") # 已实作
        self.param_tabs.setTabText(6, "🚧 目标追踪")  # 开发中

        # 初始主动触发一次 MediaPipe 全局参数页签状态显示
    ...
    def init_yoga_tab_ui(self):
        ...
        # (3) 状态显示区域 (用来显示选取的路径/视频名)
        self.label_folder_path = QLabel("尚未选择活页夹")
    ...
    def init_yolo_tab_ui(self):
        """专门负责构建 Tab 5: YOLO 参数配置的视觉组件"""
        layout = QHBoxLayout(self.yolo_tab)
        layout.setContentsMargins(15, 15, 15, 15)
        layout.setSpacing(20)

        # 📦 区块一：基础模型与源加载
        col1_box = QGroupBox("1. 基础模型与源加载")
        ...
        col1_form.addRow("", self.btn_select_yolo_video)
        col1_form.addRow("", self.label_yolo_video_path)
    ...
    def on_yolo_params_changed(self):
        ...
        # 获取当前勾选的类别清单
        current_obj_list = self.get_yolo_target_classes()

        # [进阶功能预留] 如果目前后台跑的刚好是 YOLO 相关的 Task，就可以直接呼叫它的热更新函数！
        # 这个动作我们留到写 Tab 6 启动功能时再来对接，目前先做到前端数据即时刷新。
    ...
    def on_select_folder_clicked(self):
        """开启文件夹选择器"""
        folder_path = QFileDialog.getExistingDirectory(self, "选择瑜伽测试集文件夹")
        if folder_path:
            self.label_folder_path.setText(f"已选择: {folder_path}")
            # 这里我们可以把路径存到 self 变量中备用
            self.yoga_folder_path = folder_path

    @Slot()
    def on_select_video_clicked(self):
        """开启视频文件选择器"""
        # QFileDialog.getOpenFileName 可以用来选择特定档案 (视频)
        video_path, _ = QFileDialog.getOpenFileName(self, "选择瑜伽测试视频", "", "Video Files (*.mp4 *.avi *.mov)")

        if video_path:
            self.label_video_path.setText(f"已选择: {os.path.basename(video_path)}")
            # 同样，我们将路径存到 self 变量中备用
    ...
    def on_start_yoga_clicked(self):
        ...
        if hasattr(self, 'yoga_folder_path'):
            source = self.yoga_folder_path
            source_type = SourceType.IMAGE_FOLDER
        elif hasattr(self, 'yoga_video_path'):
            source = self.yoga_video_path
            source_type = SourceType.VIDEO_FILE
        else:
            self.label_status_msg.setText("系统状态: 错误，请先选择活页夹或视频！")
            return
    ...
    @Slot(dict)
    def update_yoga_data(self, data):
        """更新瑜伽检测结果到 UI"""
        if "status" in data:
            self.label_status_msg.setText(data["status"])
    ...
    def update_raw_view(self, q_img):
        ...
        # 🛡️ 核心防线：根据当前 Label 控件的实际物理大小，进行高质量等比例缩放
    ...
    def on_tab_changed(self, index):
        ...
        # =====================================================================
        # 🎯 自动化联动机制：根据页签属性自动切换 MediaPipe 大类与模式
        # =====================================================================
        if index in [1, 2, 3]:  # 👈 选到手势音量、数字识别、PPT翻页时 (Hand 的应用)
            self.combo_mp_category.blockSignals(True)
            self.combo_running_mode.blockSignals(True)

            self.combo_mp_category.setCurrentIndex(0)  # 自动换至：手部跟踪 (Hand)
            self.combo_running_mode.setCurrentIndex(0)  # 自动换至：VIDEO (实时视频流)

            self.combo_mp_category.blockSignals(False)
            self.combo_running_mode.blockSignals(False)

            # 手动解除或锁定对应配置面板组件
            self.combo_pose_complexity.setEnabled(False)  # 手部模式下禁用姿态复杂度
            self.slider_presence_con.setEnabled(False)  # 手部模式下禁用存在置信度
            self.on_mp_global_params_changed()  # 瞬间同步一次全局参数

        elif index == 4:  # 点选「瑜珈姿势检测」页面时 (Pose 的应用)
            self.combo_mp_category.blockSignals(True)
            self.combo_running_mode.blockSignals(True)

            self.combo_mp_category.setCurrentIndex(1)  # 自动换至：姿态评估 (Pose)
            # self.combo_running_mode.setCurrentIndex(0)  # 自动换至：VIDEO (实时视频流)
            ...
            # 开启姿态专属参数的控制权限
            self.combo_pose_complexity.setEnabled(True)  # 激活 Lite/Full/Heavy 下拉框
            self.slider_presence_con.setEnabled(True)  # 激活存在置信度滑动条
            self.on_mp_global_params_changed()  # 瞬间同步一次全局参数
        ...
        if index == 0:
            ...
            self.label_status_msg.setText("系统状态: YOLO 分析就绪")


    # ====================================================================
    # 🎯 槽函数 1：命令按键按下（一站式参数审查、动态打包、切换硬件源、热插拔）
    # ====================================================================
    @Slot()
    def on_load_volume_model_clicked(self):
        ...
        # 🛑 【安全网闸防御一】：强力校验模型匹配度，防止使用者设定错误
        # 0 代表手部跟踪 (Hand)，1 代表姿态评估 (Pose)
        if self.combo_mp_category.currentIndex() != 0:
            self.label_status_msg.setText("系统错误: 当前为【手势音量控制】，模型大类必须选择 [手部跟踪 (Hand)]！")
            return  # 拦截成功！直接阻断不合法的加载请求，程序绝不闪退

        # ⚡ 【工序二】：自动切回真实 Webcam(0) 硬件媒体源
        if self.v_thread.source != 0:
            self.source_type = SourceType.CAMERA
            self.v_thread.change_media_source(0)

        # ==========================================
        # 🎯 新增需求：加载模型时，强制开启原始画面
        # ==========================================
        if not self.left_video_widget.isVisible():
            self.left_video_widget.show()                   # 1. 恢复显示左侧物理窗口
            self.btn_toggle_view.setText("👁️ 隐藏原始画面") # 2. 将右侧 Toggle 按钮文字复位
            self.v_thread.set_raw_stream_enabled(True)      # 3. 开启后台的 CPU 原图截流阀门
        ...
        # 📦 【工序三】：在真正开始执行应用时，再来搜集目前的参数设定
        task = HandVolumeTask()
        self.sync_mp_global_params(task)  # 将当前界面的最新配置一次性打包塞入 task

        # 🚀 【工序四】：正式向后台引擎注入通电卡带
        self.v_thread.switch_task(task)
        self.label_status_msg.setText("系统状态: 经过安全审查，最新 [HandModule] 参数同步并实例化成功，模型开始工作！")

    # ====================================================================
    # 🎯 槽函数 2：前台参数调整响应（仅做界面数值显示刷新，不改变后台）
    # ====================================================================
    def on_mp_global_params_changed(self):
        """
        滑块与下拉框的动态响应槽：此时它们完全在主线程内部闭环，与后端的执行绪完全隔离。
        """
        # 仅负责抓取数据更新前台 Label 文本
    ...
    # ====================================================================
    # 🎯 辅助函数 3：一站式参数动态打包（收集参数到 Task 载体中）
    # ====================================================================
    def sync_mp_global_params(self, task_instance):
        """
        专门负责在加载模型的瞬间，精准搜集和传递超参数
        """
