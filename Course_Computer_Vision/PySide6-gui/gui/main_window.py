...
class VisionWorkstationGUI(QMainWindow):
    def __init__(self):
        ...
        # ====================================================================
        # 教学挖空点 1：实例化后台执行引擎（BaseWorker）
        # 学生需手写接通发动机，指定视频源 src 和初始算法卡带 task
        # ====================================================================
        self.v_thread = BaseWorker(src=0, task=SimpleVideoTask())

        self.init_ui()

        # ====================================================================
        # 教学挖空点 2：三条专用跨线程信号电缆绑定
        # 学生需手写绑定 Signal → Slot：
        #   raw_frame_signal      → update_raw_view（左画面原始帧）
        #   processed_frame_signal → update_processed_view（右画面算法结果）
        #   data_signal           → update_business_data（状态栏业务数据）
        # ====================================================================
        ...
        # ====================================================================
        # 教学挖空点 3：启动线程引擎
        # 自动隐藏左视窗（双窗口完全对齐呈现后才隐藏）→ 开局 start()
        # ====================================================================
        self.left_video_widget.hide()
        self.btn_toggle_view.setText("👁️ 开启原始画面")
        self.v_thread.start()
        self.label_status_msg.setText("系统状态: 线程引擎已启动，视频流接入中...")
    ...
    def on_tab_changed(self, index):
        """Tab 页签切换响应，更新底部状态栏提示"""
        if not hasattr(self, "label_status_msg"):
            return
        tab_names = [
            "系统状态: MediaPipe 参数审查中",
            "系统状态: 请点击「加载模型」部署手部组件",
            "系统状态: [手势数字识别] 模块尚未开发，敬请期待",
            "系统状态: [PPT 手势翻页] 模块尚未开发，敬请期待",
            "系统状态: [YOLO 深度学习配置] 模块尚未开发，敬请期待",
        ]
        if 0 <= index < len(tab_names):
            self.label_status_msg.setText(tab_names[index])

    @Slot()
    def on_load_volume_model_clicked(self):
        """手动载入模型，触发媒体源热切换与算法卡带注入（PPT Slide 43-47）

        完整流水线:
          1. 模块可用性校检 → 2. 模型类别匹配校检 →
          3. 摄像头源切换 → 4. 重复加载防护 →
          5. 一站式中转参数 → 6. 热注入后台线程
        """
        # === 安全闸门 1: 校验 HandVolumeTask 是否可导入 ===
        if HandVolumeTask is None:
            self.label_status_msg.setText(
                "系统状态: 加载失败，[HandVolumeTask] 导入异常，请检查 qthreads/tasks/hand_volume_task.py 是否存在")
            return

        # === 安全闸门 2: 强制校验模型大类匹配 (0 = Hand 手部) ===
        if self.combo_mp_category.currentIndex() != 0:
            self.label_status_msg.setText(
                "系统警告: 当前为 [姿态评估 (Pose)] 模式，请先将模型大类切换为 [手部跟踪 (Hand)]")
            return

        # === 1. 媒体源切换: 确保从 MP4 影碟切换到实时 Webcam (id=0) ===
        if self.v_thread.camera_id != 0:
            self.v_thread.change_media_source(0)

        # === 2. 状态防护: 避免对同一任务卡带重复加载 ===
        current_task = self.v_thread.current_task
        if isinstance(current_task, HandVolumeTask):
            self.label_status_msg.setText(
                "系统状态: [HandModule] 已经加载运行中，无需重复加载")
            return

        # === 3. 一站式中转: 创建任务实例 + 同步前台参数 ===
        task = HandVolumeTask()
        self.sync_mp_global_params(task)

        # === 4. 注入并热切换: 通知后台线程安全卸载旧卡带并挂载新卡带 ===
        self.v_thread.switch_task(task)
        self.label_status_msg.setText(
            "系统状态: 加载 [HandModule] 参数同步与任务注入成功，后台线程开始推理")
    ...
    def sync_mp_global_params(self, task_instance):
        """运行时超参数动态同步契约（PPT Slide 47）

        将当前 UI 面板上的全部参数一次性批量写入 task 实例，
        供后台线程在延迟加载 HandDetector 时通过 getattr() 安全读取。
        """
        if task_instance is None:
            return

        # 运行模式: "VIDEO (实时视频流)" → "VIDEO" / "IMAGE (静态单帧)" → "IMAGE"
        mode_text = self.combo_running_mode.currentText()
        task_instance.running_mode = "IMAGE" if "IMAGE" in mode_text else "VIDEO"

        # 最大侦测对象上限
        task_instance.max_targets = self.spin_max_targets.value()

        # 三个置信度阈值（滑块 0~100 → 浮点 0.0~1.0）
    ...
    def on_mp_global_params_changed(self):
        """参数面板数值更新 → 刷新前标签 + 同步到底层正在运行的任务实例

        设计原则 (PPT Slide 41-42):
          不在滑块拖拽时实时高频写后台线程（避免竞态与锁竞争），
          而是将参数暂存在 UI 层，仅在"加载模型"时一站式中转。
          此处仍同步到已运行的 task 实例属性，供下一帧
          process() 通过 getattr 读取最新值。
        """
        ...
        # 若后台已有任务在运行，同步最新参数到其实例属性
        current_task = self.v_thread.current_task
        if current_task is not None:
            self.sync_mp_global_params(current_task)
    ...
    def closeEvent(self, event):
        """窗口关闭时安全停止后台线程并释放资源"""
        print("主窗口正在关闭，释放后台线程资源...")
        if hasattr(self, 'v_thread') and self.v_thread.isRunning():
            self.v_thread.stop()
        event.accept()
