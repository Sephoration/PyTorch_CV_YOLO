...
class YOLOMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # 窗口基础设置
        self.setWindowTitle("== YOLO11-PySide6-GUI ==")
        self.resize(1200, 700)

        self.single_view = False  # 视图模式：False = 双视图；True = 只显示右侧视图
        self.is_running = False  # 代表当前是否有任何检测在运行
        self.current_folder: str | None = None  # 当前选中的文件夹
        self.folder_images: list[str] = []  # 文件夹中的图片路径列表
        ...
        # 初始化 YOLO 侦测 / 追踪器（统一使用 yoloTracker）
        model_path = self.combo_model.currentText()
        self.tracker = yoloTracker(model_path)

        # warm-up 一次，避免第一次调用 detect 时卡顿
        dummy = np.zeros((480, 480, 3), dtype=np.uint8)  # 或 640x640 都可以
        try:
            _img, _boxes = self.tracker.detect(dummy)
    ...
    def _create_left_toolbar(self) -> QFrame:
        ...
        btn_video.clicked.connect(self.open_video_dialog)
    ...
    def _create_right_settings_panel(self) -> QFrame:
        ...
        # --- 底部按钮（占位） ---
        self.btn_batch_detect = QPushButton("批次检测")
        self.btn_stop = QPushButton("停止检测")
        btn_save_result = QPushButton("保存结果（待实现）")

        layout.addWidget(self.btn_batch_detect)
        layout.addWidget(self.btn_stop)
        layout.addWidget(btn_save_result)

        # 绑定批次检测事件
        self.btn_batch_detect.clicked.connect(self.run_batch_detection)
    ...
    def run_batch_detection(self):
        ...
        if self.is_running:
            if self.status_label:
                self.status_label.setText("批次检测已经在进行中……")
            return

        # 没选文件夹就提示
        if not self.current_folder or not self.folder_images:
            if self.status_label:
                self.status_label.setText("请先点击 Folder 按钮选择图片文件夹。")
            return

        total = len(self.folder_images)
        if total == 0:
            if self.status_label:
                self.status_label.setText("当前文件夹中没有可用图片。")
            return
        ...
        if self.status_label:
            self.status_label.setText(
                f"开始批次检测，共 {total} 张图片（按停止键结束）……"
            )
        try:
            # 无限循环批次检测
            while self.is_running:
                for idx, img_path in enumerate(self.folder_images, start=1):
                    # 若途中按停止键
                    ...
                    if img_bgr is None:
                        if self.status_label:
                            self.status_label.setText(f"读取失败：{img_path}，继续下一张……")
                        continue
                    img_raw = img_bgr.copy()
                    # 使用统一 YOLO 侦测接口
                    try:
                        img_detected, pred_boxes = self.tracker.detect(img_bgr)
                    except Exception as e:
                        if self.status_label:
                            self.status_label.setText(f"检测失败：{img_path}，错误：{e}")
                        continue
                    # 显示左右图
                    self.show_image(self.view_left, img_raw)
                    self.show_image(self.view_right, img_detected)
                    # 更新进度条
                    ...
                    if self.status_label:
                        self.status_label.setText(
                            f"批次检测：第 {idx}/{total} 张 ({os.path.basename(img_path)})"
                        )
                    # GUI 刷新
            ...
            if self.status_label:
                self.status_label.setText("批次检测已停止。")
            if self.progress_bar:
                self.progress_bar.setValue(0)
    ...
    def stop_all_detection(self):
        ...
        if self.status_label:
            self.status_label.setText("停止检测请求已发送")
    ...
    def open_video_dialog(self):
        ...
        # ✅ 直接进入连续视频检测，不做第一帧单独显示
        self.run_video_detection(file_path)
    ...
    def open_image_dialog(self):
        ...
        if img_bgr is None:
            if self.status_label:
                self.status_label.setText("Failed to read image file.")
            return

        # 保留一份原始图像（左边显示用）
        img_raw = img_bgr.copy()

        # 3. 调用统一的 YOLO 侦测接口（不带追踪 ID）
        try:
            img_detected, pred_boxes = self.tracker.detect(img_bgr)
        except Exception as e:
            if self.status_label:
                self.status_label.setText(f"Image detection failed: {e}")
            return
        # 4. 显示图像（左：原图；右：检测结果）
        self.show_image(self.view_left, img_raw)
        self.show_image(self.view_right, img_detected)
        ...
        if self.status_label:
            self.status_label.setText(
                f"Detection finished ✓  (Objects: {len(pred_boxes)})"
            )
    ...
    def open_folder_dialog(self):
        ...
        if self.status_label:
            self.status_label.setText(
                f"已选择文件夹：{self.current_folder}，共 {count} 张图片，待批次检测"
            )
    ...
    # ======================================================================
    def show_image(self, label: QLabel | None, img_bgr):
        if label is None or img_bgr is None:
            return
    ...
    def run_video_detection(self, video_path):
        ...
        while self.is_running:
            ret, frame = cap.read()
            # 如果读不到帧，说明到结尾了：把播放指针重设到开头，然后继续下一轮
            if not ret:
            ...
            # YOLO 侦测
            img_detected, pred_boxes = self.tracker.detect(frame)
            ...
            # 更新状态栏
            if self.status_label:
                self.status_label.setText(f"Video detecting... Objects: {len(pred_boxes)}")
            ...
            if self.status_label:
                self.status_label.setText("Video detection stopped.")
