...
class VisionWorkstationGUI(QMainWindow):
    def __init__(self):
        ...
        # 1. 实例化多线程视觉引擎，并将媒体源指定为指定的测试视频
        self.v_thread = BaseWorker(camera_id="videos/cxk.mp4")

        # 2. 建立跨线程的“通信电缆”：将后端的发射信号绑定到前端的接收槽函数
        ...
        # 3. 注入我们刚刚写好的简单视讯处理卡带 (SimpleVideoTask)
        self.current_task = SimpleVideoTask()
        self.v_thread.switch_task(self.current_task)

        # 4. 点火开机：正式启动后台线程的 run() 死循环
        self.v_thread.start()
    ...
    def update_business_data(self, data):
        if "status" in data:
            self.label_status_msg.setText(f"系统状态: {data['status']}")
